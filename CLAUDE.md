...
- **Tablero**: matriz `ROWS × COLS` de enteros. `0` = vacía; `1–14` = índice compartido en `COLORS` y `PIECES`. El mismo entero identifica pieza y color, así que añadir una pieza obliga a extender ambos arrays en paralelo (y el rango de `randomPiece()`, que sigue acotado a `1–8`). Reparto: `1–7` piezas estándar, `8` tuerca, `9–13` power-ups (`FIRST_POWER`/`LAST_POWER`), `14` comodín (`WILDCARD`, sin entrada en `PIECES` — nunca es pieza).
